...
>
> ⚠️ **Наполнение `candidate_sources` сейчас ручное.** Штатный `discover-sources` (DuckDuckGo)
> заблокирован — эндпоинт отдаёт HTTP 202 (антибот) при любом User-Agent → 0 кандидатов. Пока
> домены модерируются вручную (поиск первоисточника → проверка статикой на JSON-LD/SPA → INSERT
> в `candidate_sources`). Лечение — заменить `DuckDuckGoProvider` на Serper/Brave API за тем же
> ABC `SearchProvider` (см. «Что осталось сделать»).
...
seeds.yaml (provider + параметры)
  ...
  │       source_url = game["url"] или schedule_url города
  │
...
source.url (URL страницы-листинга)
  ...
  │   _resolve_event_url(event_url, source.url) → source_url события
...
#### `vk_posts` — посты VK → префильтр → LLM (1 вызов/группу)
...
source.vk_groups = ["perm_afisha", "kudago_perm", ...]
  ...
  │
  │   Берём до 20 кандидатов. Формируем батч:
  │   «=== POST https://vk.com/wall{owner}_{id} ===\n{text}\n\n»
  │
  │   extractor.extract_many(batch_doc, group_url)
  │   ← ОДИН LLM-вызов на всю группу
  │   → list[ParsedEvent]  (LLM читает маркер «=== POST url ===» → event_url)
  │
  │   Для каждого ParsedEvent:
  │   ├── _is_past_event(date) → пропустить
  │   └── _resolve_event_url(event_url, group_url) → source_url
  │         event_url → прямая ссылка на пост (vk.com/wall-…)
  │         если пустой/мусорный → фолбэк на ссылку группы
  │
...
#### `telegram_posts` — Telegram-каналы → префильтр → LLM (1 вызов/канал)
...
source.telegram_sources = [{channel, source_type, priority, enabled}, ...]
  ...
  │
  │   extractor.extract_many(batch_doc, channel_url)
  │   ← ОДИН LLM-вызов на весь канал
  │   → list[ParsedEvent]
  │
  │   Для каждого ParsedEvent:
  │   ├── _is_past_event(date) → пропустить
  │   └── _resolve_event_url(event_url, channel_url) → source_url
  │         event_url → прямая ссылка на пост (t.me/{channel}/{id})
  │         если пустой/мусорный → фолбэк на ссылку канала
  │
  │   save_raw_document() для обработанных постов
  │
  ▼ EventRow[] → merge → upsert
...
- `source_url` события: `game.get("url")` из ответа API; если null — страница расписания города (`schedule_url`)
...
- **`SearchProvider`** (ABC) — интерфейс поисковика. Реализация `DuckDuckGoProvider` (HTML, без ключа,
  ...
  > ⚠️ **`DuckDuckGoProvider` сейчас не работает** (проверено 2026-06): HTML-эндпоинт отдаёт HTTP 202
  > (антибот-челлендж) → 0 результатов. Контур Discovery наполняется вручную, cron в
  > `discover_sources.yml` на паузе. Нужен переход на keyed-провайдер (Serper/Brave) — backlog.
...
Осталось:
- **Починить Discovery (DuckDuckGo → Serper/Brave)** 📋 — штатный `discover-sources` блокируется
  (HTTP 202), `candidate_sources` пополняется вручную. Без рабочего поиска нечем питать generic
  на длинном хвосте. Реализовать `SerperProvider` за ABC `SearchProvider`, ключ в env, вернуть cron.
- **Включить generic для Сочи** 📋 — после починки Discovery и набора одобренных доменов Сочи.
