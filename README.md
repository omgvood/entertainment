...
| **Миграция Groq → gpt-oss** | Groq декоммиссит `llama-3.1-8b-instant` и `llama-3.3-70b-versatile` **16.08.2026**. Дефолт Groq переведён на `openai/gpt-oss-120b` (замена 70b; у gpt-oss-20b/120b одинаковый free-tier 8K TPM, взяли мощнее). Новый потолок 8K TPM **ниже** прежних 12K → пачка из 5 постов даёт `413`; `extract_many` ловит 413 и делит батч пополам по границам постов `=== POST <url> ===`, переизвлекая рекурсивно (`groq.batch_split`) — без глобального снижения `POST_BATCH_SIZE` | `config.DEFAULT_MODELS`, `groq_extractor.GroqExtractor`/`_split_posts`, `extraction/_errors.is_request_too_large` |
...
│   │   │   ├── groq_extractor.py   — Groq openai/gpt-oss-120b (+ авторазбиение батча на 413)
...
- `POST_BATCH_SIZE` (дефолт 5) — сколько VK/TG-постов склеивать в один LLM-вызов. Оптимально для
  Gemini (лимиты выше). При переключении **основного** провайдера на Groq (редкий кейс) рекомендуется
  `POST_BATCH_SIZE=3` (free-tier gpt-oss = 8K TPM, пачка из 5 даёт 413). При обычном использовании
  (Groq — только фолбэк) менять не нужно: `GroqExtractor` сам делит батч пополам на 413
...
> `openai/gpt-oss-120b` на free-tier имеет TPM 8000 — пачка из 5 постов (~11k токенов) не лезет
> (`413 Request too large`). Это **не** фатально: `GroqExtractor.extract_many` ловит 413 и делит
> батч пополам по границам постов (`groq.batch_split` в логах), переизвлекая половины рекурсивно.
...
- **`extract_many()`:**
  ...
  - **Авторазбиение на 413.** TPM gpt-oss-free (8K) ниже прежних 12K у 70b → пачка из 5 постов
    (~11k токенов) даёт `413 Request too large`. `extract_many` ловит 413 (`_errors.is_request_too_large`)
    и через `_split_posts` делит вход пополам по границам маркеров `=== POST <url> ===`, рекурсивно
    переизвлекая половины (`groq.batch_split` в логах). База рекурсии: `< 2` постов (один пост или
    листинг без маркеров) → `ExtractorError` (делить нечего; для гигантского листинга — на будущее
    truncation `max_chars`, пока такие идут через JSON-LD)
...
> на free-tier имеет TPM=8000 — пачка из 5 постов (~11k токенов) не лезет (`413 Request too large`).
> Раньше для groq-фолбэка спасал больший TPM у 70b (12k), но 70b декоммиссится 16.08.2026. Теперь
> 413 обрабатывается **автоматически**: `GroqExtractor.extract_many` делит батч пополам по границам
> постов и переизвлекает рекурсивно (`groq.batch_split`); глобально снижать `POST_BATCH_SIZE` не нужно.
...
- **`discover_sources(city, supabase, *, settings, dry_run)`** — точка входа: гоняет типовые запросы
  ...
  агрегирует домены, считает score, пишет в БД. Возвращает `(candidates, all_providers_failed)` —
...
- **Скоринг кандидата:** `+3` если на странице найден JSON-LD типа `Event`; `+2` если в пути есть
  ...
  `−5` (`_AGGREGATOR_PENALTY`) если домен — федеральный агрегатор/билетная платформа из
  `_AGGREGATOR_PENALTY_DOMAINS` (`afisha.ru`, `ticketland.ru`, `kassir.ru`, `timepad.ru`).
  Штраф применяется **последним**, после всех начислений (иначе новые бонусы нивелировали бы его),
  и ловит поддомены (`perm.afisha.ru`) через суффиксное сравнение `endswith("." + d)` — без ложных
  совпадений (`myafisha.ru` не штрафуется).
- **Зачем штраф (`candidate.aggregator_penalty`):** агрегаторы стабильно попадают в топ выдачи и
  набирают высокий скор (отличный JSON-LD `+3`, event-path `+2`, частота `+1..n`), вытесняя локальных
  организаторов в очереди модерации — хотя бесполезны (дублируют Timepad/`seeds.yaml`,
  `unique_events_ratio ≈ 0`). Штраф `−5` ≥ суммы двух сильнейших структурных бонусов (`+3`+`+2`),
  поэтому гарантированно опускает агрегатор ниже сопоставимого организатора. Это **не** жёсткий фильтр
  (в отличие от `_IGNORE_DOMAINS`): домен остаётся кандидатом с пониженным score — виден в
  `discovery-health` и при модерации, модератор может вручную перевести его в `approved`. Лог пишет
  `score_before`/`score_after`/`penalty` для аналитики. Список ведётся вручную по факту появления
  доменов в логах; `timepad.ru` — временно, пока `_known_domains()` не научится ловить
  `direct_api`-источники без `url` в seeds.
