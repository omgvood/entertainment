...
- `playwright_listing` — то же что `batch_listing`, но страница рендерится headless-браузером (Playwright) перед извлечением. **Для SPA** (Nuxt/Next/React), где статический HTML пуст: браузер исполняет JS, отдаёт готовый DOM → дальше та же цепочка JSON-LD → LLM. Запущен для Перми: `permopera.ru`, `permm.ru`
...
организаторов, а не реагрегаторы). Домены `permopera.ru` и `permm.ru` — тоже SPA (Nuxt),
статический HTML у них пуст, поэтому generic их не брал; теперь они подключены отдельными
источниками через режим **`playwright_listing`** (см. выше). Домены `permtuz.ru` /
`teatr-umosta.ru` остаются кандидатами на тот же режим (добавляются парой строк в `seeds.yaml`
после проверки `/afisha`-URL).
...
> Запущен для Перми: `permopera.ru` (концерты, priority 55), `permm.ru` (выставки, priority 55).
...
- **Спец-источники для SPA-площадок** ✅ — `permopera.ru` / `permm.ru` (афиша за JS) подключены
  через режим `playwright_listing` (Playwright-рендер → batch_listing). Следующие кандидаты на тот
  же режим: `permtuz.ru`, `teatr-umosta.ru`.
