...
> Боевой прогон (`discover-sources --city perm --dry-run` + `discovery-health`) требует `SERPER_API_KEY`
> в окружении — запускается локально/в CI перед мержем для финальной проверки.
