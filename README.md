...
Python-парсер (parser/)
    ...
    ├── vk_posts: посты VK-сообществ → префильтр → LLM (1 вызов/пост)  [сервисный ключ]
    ├── vk_events: события-сообщества → ParsedEvent без LLM  [отключён: нужен user-токен]
    ├── telegram_posts: посты публичных каналов (t.me/s/) → префильтр → LLM (1 вызов/пост)
...
| **VK-источники** | `vk-posts` (посты кураторских сообществ + префильтр + LLM, 1 вызов/пост) — работает на сервисном ключе. `vk-events` (события-сообщества без LLM) готов, но отключён: `groups.search` требует user-токен | `sources/vk.py`, режимы `vk_posts`/`vk_events` |
| **Telegram-источник** | `telegram-posts` (посты публичных каналов через веб-превью `t.me/s/`, без авторизации) — fetch → префильтр → LLM (1 вызов/пост). Каналы — объекты с `source_type`/`priority`/`enabled`; провайдер за абстракцией (HTML сейчас, MTProto позже) | `sources/telegram.py`, режим `telegram_posts` |
...
│   │   ├── test_extractors.py      — GeminiExtractor, GroqExtractor (mock LLM)
...
- `vk_posts` — посты со стен `vk_groups`: префильтр (дата/маркеры/билеты) → отдельный `extract_many` на каждый пост (параллельно, с ограничением `_POST_CONCURRENCY`)
- `telegram_posts` — посты публичных каналов `telegram_sources` (веб-превью `t.me/s/`, без авторизации): префильтр (строгость по `source_type`) → отдельный `extract_many` на каждый пост
...
- **`vk_posts`** — `fetch_wall_posts(group)`: посты со стен `vk_groups`. **Работает на сервисном
  ...
  и фильтр свежести 14 дней; дедуп постов через `raw_documents` (хеш текста). Каждый пост-кандидат
  (≤20/группу) обрабатывается **отдельным** `extract_many` — параллельно, с ограничением `_POST_CONCURRENCY`.
  Так `source_url` фолбэчит на ссылку самого поста, а не на группу. Сам пост помечается маркером
  `=== POST <url> ===` (совместимость с промптами) → `event_url` → `resolve_event_url`; фолбэк на `post_url`.
...
- **Режим `telegram_posts`** (`pipeline._run_telegram_posts_source`): зеркало `vk_posts`. Каналы —
  ...
  дедуп через `raw_documents`, **отдельный** `extract_many` на каждый пост (параллельно, `_POST_CONCURRENCY`).
...
#### `vk_posts` — посты VK → префильтр → LLM (1 вызов/пост)
...
source.vk_groups = ["perm_afisha", "kudago_perm", ...]
  ...
  │
  │   Берём до 20 кандидатов. Каждый пост — отдельная задача:
  │   «=== POST https://vk.com/wall{owner}_{id} ===\n{text}»
  │
  │   asyncio.gather(extractor.extract_many(post_doc, post_url) ...)
  │   ← ОТДЕЛЬНЫЙ LLM-вызов на каждый пост, параллельно (Semaphore _POST_CONCURRENCY)
  │   → list[ParsedEvent]  (LLM читает маркер «=== POST url ===» → event_url)
  │
  │   Для каждого ParsedEvent:
  │   ├── _is_past_event(date) → пропустить
  │   └── resolve_event_url(event_url, post_url) → source_url
  │         event_url → прямая ссылка на пост (vk.com/wall-…)
  │         если пустой/мусорный → фолбэк на post_url самого поста
  │
...
#### `telegram_posts` — Telegram-каналы → префильтр → LLM (1 вызов/пост)
...
| temperature | 0 | 0 | 0.3 |
| Ключ (env) | `GEMINI_API_KEY` | `GROQ_API_KEY` | `OPENROUTER_API_KEY` |
