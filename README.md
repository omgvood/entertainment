...
**Стек:** Python 3.10+ (парсер) · PostgreSQL / Supabase (хранилище) · Next.js 16 SSG (фронтенд) · GitHub Actions (CI/CD) · Vercel (хостинг)
...
|-----------|-----------|--------|
| Концерты | Timepad + generic (Пермская филармония) + permopera (direct_api) | ✅ |
| Театр | Timepad + generic (Театр-Театр) + permopera (direct_api) | ✅ |
| Выставки | Timepad + VK + Telegram + permm (direct_api) | ✅ |
...
| Квесты | 2ГИС | ✅ |
| Боулинг | 2ГИС | ✅ |
| Спорт (места) | 2ГИС (боулинг/бильярд/картинг/скалодром/батут/теннис) | ✅ места-«всегда»; матчи спортклубов — в работе (VK-группы клубов; сайты клубов SPA — см. `seeds.yaml`) |
...
| **SPA-листинги через Playwright** | Режим `playwright_listing`: афиша SPA-сайтов (Nuxt/Next/React) рендерится headless-браузером, затем идёт в обычную цепочку `batch_listing` (JSON-LD → LLM). Закрывает первоисточники-организаторы, чей статический HTML пуст. Хеш-дедуп считается по **видимому тексту** (динамические скрипты Nuxt игнорируются). Режим доступен, но сейчас ни один источник на нём не запущен — `permopera.ru`/`permm.ru` переведены на `direct_api` (см. ниже) | `sources/playwright_fetcher.py`, `pipeline._run_batch_source` (`use_playwright`), `pipeline._hash_html`, `parse.yml` |
| **SPA с открытым JSON → direct_api** | `permopera.ru` и `permm.ru` — SPA, но отдают данные открытыми JSON-эндпоинтами под капотом. Подключены как `direct_api` (без браузера, без LLM): надёжнее и быстрее `playwright_listing`, не тратят квоту Gemini, сняли проблемы SSL/404. permm — чистый JSON (`/json/exhibitions/active`, `/json/events/sub/home`); permopera — HTML-в-JSON (`{"content": "<карточки>"}`), парсится `selectolax` по data-атрибутам. Адрес/площадка — из seeds (`venue_name`/`address`), цена 0/«по билетам» | `sources/permm.py`, `sources/permopera.py`, `pipeline._fetch_direct_api_items`, `config.SourceConfig.venue_name/address` |
...
| **Фильтр не-событий** | Новости городских пабликов, объявления служб, промо, розыгрыши, голосования и open call просачивались в `other` (~27% карточек `/perm/other` на 2026-09-18, в основном из `permactive`). Три слоя: (1) строгость VK-префильтра по группам `vk_source_types` (`permactive: aggregator`); (2) правило `NON_EVENT_INSTRUCTIONS` в batch-промптах; (3) постфильтр `is_non_event` по заголовку в `to_event_row` (счётчик `skipped_non_event` → `source_health`) — эвристический guard только по узким маркерам и только для social/generic, широкие случаи оставлены префильтру и промпту. Правило модели: open call / приём заявок, розыгрыши, голосования, промо — не события. Приток перекрыт (смержено в master). Накопленные 15 строк (11 по предикату + 4 ручной дочистки, включая находку из Сочи) подтверждены пользователем и зафиксированы в миграции; применение к боевой БД — после первого прогона крона новым кодом (нужно выделить дельту, собранную ещё старым кодом, см. `docs/superpowers/plans/2026-09-18-non-event-filter.md`) | `seeds.yaml`, `extraction/prompts.py`, `validator.is_non_event`, `pipeline._safe_to_event_row`, миграция `…_remove_non_events_social.sql` |
...
| **Неоновый редизайн фронта** | Тёмная неоновая тема: токены в `globals.css`, единые стили бейджей/плейсхолдеров событий. Главная перестроена с «сетка + сайдбар» на **hero + горизонтальный `FilterBar` + лента дат** (`Sidebar.tsx` удалён; секции «Сегодня / Завтра / Дальше» из первой версии редизайна заменены лентой дат — см. «Лента дат и серии» ниже). Карточка показывает площадку, описание и теги; битая картинка источника падает на emoji-заглушку | `web/app/globals.css`, `lib/event-styles.ts`, `components/CityView.tsx`, `components/FilterBar.tsx`, `components/CardImage.tsx` |
...
│   │   │   ├── groq_extractor.py   — Groq openai/gpt-oss-120b (batch-413/TPM → rate-limit → фолбэк)
...
│   │       ├── playwright_fetcher.py — рендер SPA-страницы через Playwright → HTML
│   │                                  (режим playwright_listing; опц. dep [playwright])
│   │       ├── timepad.py          — Timepad API, широкая афиша; тип по категории (без LLM)
│   │       ├── permm.py            — Музей ПЕРММ: JSON-эндпоинты /json/* → ParsedEvent (без LLM)
│   │       ├── permopera.py        — Театр оперы и балета: афиша месяца (HTML-в-JSON) → selectolax (без LLM)
...
│   ├── scripts/
│   │   └── notify_warnings.py      — Telegram-алерт по parse_warnings_<city>.json (вызов из GHA, stdlib)
│   ├── tests/                      — pytest тесты
...
│   │   ├── CityView.tsx            — hero + FilterBar + лента дат + серии + SEO-описание; состояние (дата/тип/цена/поиск) в URL и sessionStorage
│   │   ├── FilterBar.tsx           — горизонтальная панель фильтров (поиск, чипы типов — пустой выбор = все, «× Сбросить», поповер цены «Ещё фильтры»)
│   │   ├── DateStrip.tsx           — лента дат (Сегодня/Завтра/Выходные/отдельные дни/Все даты) со счётчиком серий
│   │   ├── EventCard.tsx           — карточка события; для серии показывает «ещё N дат до …»
...
│   │   ├── filters.ts              — фильтрация серий по типу/цене/дате + typesByFrequency
...
| `--mode per_url\|batch_listing\|playwright_listing\|direct_api\|vk_events\|vk_posts\|telegram_posts\|generic` | Переопределить режим извлечения |
...
**Режимы извлечения:**
- `direct_api` — вызов внешнего API (QuizPlease / 2ГИС / Timepad / KudaGo / ПЕРММ / Театр оперы), маппинг JSON → `ParsedEvent` без LLM
...
- `vk_posts` — посты со стен `vk_groups`: префильтр (дата/маркеры/билеты; строгость по группе — `vk_source_types`, по умолчанию social) → `extract_many` пачками по объёму текста (`POST_BATCH_MAX_CHARS`, потолок `POST_BATCH_SIZE` постов) (параллельно, с ограничением `_POST_CONCURRENCY`)
...
**`_run_batch_source(source, extractor, supabase, city, dry_run, *, use_playwright=False)`**
...
- Batch-вариант эффективнее для статических страниц с полным расписанием; SPA-сайты (Nuxt/Next) требуют `direct_api`, `playwright_listing` или специализированный источник
- **`use_playwright=True`** (режим `playwright_listing`) — шаг fetch идёт через `playwright_fetcher.render_page()` (headless-браузер рендерит JS), остальная логика идентична. Хеш-дедуп при этом считается по видимому тексту (`_hash_html(html, playwright=True)`), а не по сырому DOM — иначе динамические блоки Nuxt (`__NUXT_DATA__`, токены, классы гидратации) ломали бы пропуск неизменных листингов
...
- HTTP 400/401/403 (битый/отозванный токен) ловится отдельно от прочих ошибок: пишет
  понятное предупреждение в `sub.warnings` (→ Telegram-алерт), а не теряется в общем `except`
...
- `record_source_health(client, source, city, events_found, errors, duration_sec, last_error)` —
  ...
  `last_error` теперь заполняется первым предупреждением источника (напр. текст про HTTP 403) либо `NULL`
  при чистом прогоне — видно прямо в БД, какой источник и чем именно «болеет»
...
- `text_score(a, b)` — `max(посимвольное сходство, вложенность токенов, Jaccard)`. Каждая
  ...
  Она же выключается для **перечня** (`is_enumeration`): в `Мастер-классы: «Единорог»,
  «Планета», «Ёжик»` вложены три РАЗНЫХ мастер-класса, а не одна переформулировка.
  Признак перечня — два кавычечных сегмента подряд, между которыми только запятая или «и»;
  осмысленный текст между кавычками перечня не образует (`«АРТ-парк»: Мастер-класс «Цветы
  из бумаги»` — цикл и его занятие, обычный дубль), вложенные кавычки считаются одним
  сегментом (`Праздник «Яблочный Спас в «Хохловке»»`).
...
- `cluster_events(rows, merge_threshold, report_threshold)` — блокировка по `city+date`
  ...
  Перед union-find отсекаются **зонтичные заголовки** (`_drop_umbrella_edges`): если короткое
  название вложено в несколько длинных, не похожих между собой (`День Строгановых` ⊂
  `…: книжная выставка` и ⊂ `…: показ фильма`), это программа дня, а не формулировки одного
  анонса — его рёбра-вложенности выбрасываются. Попарно такой случай неотличим от настоящего
  дубля (`Культурная среда` ⊂ `Культурная среда: бесплатный вход…` — один ребёнок, слияние
  остаётся), поэтому решает вид всего кластера. Скобочные уточнения различием детей не
  считаются: `…(Воскресенье)` и `…(Спешилова 111/3)` — то же событие.
...
организаторов, а не реагрегаторы). Домены `permopera.ru` и `permm.ru` — тоже SPA, но у них нашлись
открытые JSON-эндпоинты, поэтому они подключены не через рендер браузером, а через **`direct_api`**
(`sources/permopera.py`, `sources/permm.py` — см. ниже): без LLM и без Playwright. Домены
`permtuz.ru` / `teatr-umosta.ru` остаются кандидатами на `playwright_listing` или, если у них тоже
найдётся JSON-API, на `direct_api`.
...
> `openai/gpt-oss-120b` на free-tier имеет TPM 8000, а фиксированный оверхед batch-вызова
> (system-промпт со схемой ~2k + резерв `max_tokens`) сам близок к лимиту → batch почти всегда
> даёт `413 … TPM`. Разбиением входа это **не** лечится (лимит не про размер входа), поэтому 413
> относится к rate-limit и батч уходит на фолбэк (Gemini). Single-extract на Groq укладывается.
...
Использует Groq SDK с моделью `openai/gpt-oss-120b` (дефолт; переопределяется `GROQ_MODEL`).
Прежний дефолт `llama-3.3-70b-versatile` Groq декоммиссит **16.08.2026** (вместе с `llama-3.1-8b-instant`),
замена для 70b — `openai/gpt-oss-120b`, для 8b — `openai/gpt-oss-20b`. У обеих gpt-oss free-tier лимиты
одинаковы (30 RPM / 1K RPD / **8K TPM** / 200K TPD), поэтому взяли более мощную 120b.
...
- **`extract_many()`:**
  ...
  - `max_tokens=4000` (а не 8000): пачке постов столько вывода не нужно, а резерв `max_tokens`
    целиком учитывается в TPM-счётчике Groq — лишний резерв сам провоцирует 413
  - **413/TPM → rate-limit → фолбэк.** На free-tier (8K TPM) фиксированный оверхед (system-промпт
    ~2k + `max_tokens`) сам близок к лимиту, поэтому batch почти всегда даёт `413 … tokens per
    minute`. Это лимит, а не размер входа (разбиением не лечится): `_errors.is_rate_limit` относит
    такой 413 к rate-limit, `extract_many` поднимает `RateLimitError`, и `FallbackExtractor` уводит
    батч на Gemini. Подтверждено dry-run’ом (`--provider groq`): batch-источники падали в `413 … TPM`
    и переключались на Gemini (`fallback.switch`), пайплайн не падал
...
> отдают пустой скелет — ноль событий. Такие источники переводятся на `direct_api` (если есть API)
> или на режим **`playwright_listing`** — тот же алгоритм, но шаг fetch рендерит страницу в браузере.

---

#### `playwright_listing` — рендер SPA в браузере → batch_listing

Для SPA-сайтов (Nuxt/Next/React), где `httpx` видит пустой скелет. Отличается от `batch_listing`
**только шагом fetch**: вместо `httpx.get` страница открывается в headless Chromium, JS исполняется,
снимается готовый DOM. Дальше — та же цепочка JSON-LD → LLM.

```
source.url (SPA-листинг афиши)
  │
  ▼ playwright_fetcher.render_page(url)
  │   ├── headless Chromium (ленивый импорт; опц. dep [playwright])
  │   ├── goto(url, wait_until="networkidle")  ← ждём окончания JS-рендера
  │   │     └── TimeoutError (тяжёлая аналитика) → WARNING, берём что отрендерилось
  │   ├── проверка HTTP-статуса (goto не бросает на 4xx/5xx) → RuntimeError при ≥400
  │   ├── scrollTo(0, scrollHeight) + пауза  ← триггер lazy-load карточек
  │   ├── try/finally: context.close + browser.close  ← без зомби-процессов Chromium
  │   └── page.content() → готовый HTML
  │
  ▼ _hash_html(html, playwright=True)   ← хеш ТОЛЬКО видимого текста (selectolax):
  │   вычищает script/style/meta/template/hidden + нормализует пробелы.
  │   Иначе динамические блоки Nuxt (__NUXT_DATA__, токены) меняли бы хеш каждый прогон.
  │
  ▼ ДАЛЕЕ — идентично batch_listing (JSON-LD → LLM-фолбэк → raw_documents → merge → upsert)
```

> Режим доступен, но сейчас не используется: `permopera.ru`/`permm.ru` переведены на `direct_api`
> (нашлись открытые JSON-эндпоинты — дешевле и надёжнее браузерного рендера).
> CI (`parse.yml`) ставит браузер через `pip install -e '.[playwright]'` + `playwright install
> chromium --with-deps`; бинарники Chromium кэшируются (`actions/cache`) по хешу `pyproject.toml`.
> `_apply_stealth` (playwright-stealth, если установлен) снижает риск антибот-детекта.
...
> ⚠️ **Spurious `date='always'` от VK/Telegram.** LLM иногда ставит `date='always'` постам без чёткой
> даты (регулярный ивент, «каждую пятницу», нет конкретной даты в тексте). Такие события попадают
> во фронтенд-выборку `date='always' OR date>=today` и в секцию «постоянные места», куда не должны.
> Существующая миграция `remove_always_from_events` чистит только `twogis-%`-строки и **не трогает**
> VK/TG. Пока нет автоматической очистки — такие записи удаляются вручную через Supabase Table Editor
> (`DELETE FROM events WHERE source LIKE 'vk-%' AND date='always'`). Долгосрочное решение — в roadmap п. 15.
...
| max_tokens batch | 20 000 | 4 000 | 8 000 |
| temperature | 0 | 0 | 0.3 |
...
> без кредитов отдаёт `402`, поэтому в цепочку по умолчанию не включён. Groq `openai/gpt-oss-120b`
...
### `sources/playwright_fetcher.py` — рендер SPA-страницы (режим `playwright_listing`)

Контур **EVENTS**, не venues (в отличие от `playwright_2gis.py`). Универсальный рендер одного URL:
поднимает headless Chromium, ждёт окончания JS-рендера и возвращает готовый HTML, который
`pipeline._run_batch_source(use_playwright=True)` гонит по обычной цепочке `batch_listing`
(JSON-LD → LLM). Не знает структуры конкретного сайта — этим отличается от 2ГИС-скрейпера, который
парсит карточки по CSS-селекторам. Зависимость опциональная (`pip install -e '.[playwright]'`),
импорт playwright ленивый.

**`render_page(url, *, timeout_ms=30_000) → str`** — единственная публичная функция:
1. `goto(url, wait_until="networkidle")` — ждёт, пока Nuxt/Next дорисует афишу (нет сетевой активности 500 мс)
2. **Таймаут `networkidle`** (тяжёлая аналитика держит соединение) → `WARNING`, берём что уже отрендерилось — не падаем
3. **Проверка HTTP-статуса**: `page.goto` (в отличие от `httpx`) не бросает на 4xx/5xx — иначе скормили бы LLM кастомную страницу 404 и сожгли токены. При статусе ≥400 → `RuntimeError`
4. **Скролл вниз** (`scrollTo(0, scrollHeight)` + пауза) — триггерит lazy-load карточек на длинных листингах
5. **`try/finally`** вокруг `browser`/`context` — `RuntimeError(HTTP)` иначе оставил бы зомби-процесс Chromium в долгоживущем прогоне
6. `page.content()` → полный DOM

**Почему `_apply_stealth` дублируется** (а не импортируется из `playwright_2gis`): модуль самодостаточен
и не ломается при правках 2ГИС-скрейпера. Это 10 строк — `playwright-stealth`, если установлен.

**Стабильность хеш-дедупа** — ключевая тонкость SPA. `pipeline._hash_html(html, playwright=True)`
хеширует **только видимый текст** (через `selectolax`: вычищает `script`/`style`/`meta`/`link`/
`template`/`input[hidden]`, нормализует пробелы). Для сырого DOM хеш «мигал» бы каждый прогон —
Nuxt встраивает динамические `<script id="__NUXT_DATA__">` с токенами/таймстампами и
рандомизированные классы гидратации, и пропуск неизменных листингов (экономия LLM) сломался бы.

**Диагностика** при `extracted=0`: `render_page` логирует `html_len`; если контента мало —
открой сайт в браузере → DevTools Console → `copy(document.body.outerHTML)`, проверь наличие
`<script type="application/ld+json">` или текста событий. При пустоте — сайту нужен более глубокий
скролл (цикл прокрутки по аналогии с `_scroll_results` в `playwright_2gis.py`).
...
> ⚠️ **Диагностика 403 в GHA.** Протухший `TIMEPAD_TOKEN` не валит весь прогон `parse.yml`
> (остальные источники отрабатывают), поэтому раньше сбой был «тихим»: джоба зелёная, событий
> Timepad нет. Теперь HTTP 400/401/403 от direct_api-источника пишет предупреждение в
> `parse_warnings_<city>.json`, и шаг **Send warning alert** шлёт Telegram (`timepad-perm: HTTP 403 —
> проверь токен/ключ API`). Тот же текст оседает в `source_health.last_error`.
> Это **протухший `TIMEPAD_TOKEN` в Secrets**, а не IP-блок (на локальной машине тот же код работает).
> Как проверить: скопировать значение Secret и выполнить
> `curl -H "Authorization: Bearer <token>" "https://api.timepad.ru/v1/events?limit=1"` —
> 200 = токен живой, 401/403 = перевыпустить на timepad.ru и обновить Secret.

### `sources/permm.py` — Музей ПЕРММ (direct_api)

**`PermMuseumClient`** — клиент к открытым JSON-эндпоинтам permm.ru (SPA, статический HTML пуст):
`GET /json/exhibitions/active` (активные выставки) и `GET /json/events/sub/home` (ближайшие
события — экскурсии, лекции). LLM и браузер не нужны.

- `search(event_type, venue_name, address)` → `list[(ParsedEvent, source_url)]`. Дедуп по `id`.
- `_map_item`: `title_extended` → title, `sub_title_extended[:500]` → description,
  `media_url_extended` → image_url (хелпер `_resolve_image` достаёт встроенный URL из
  tilda-прокси `/storage/tilda/https://...`), `source_url = https://permm.ru{url}`.
- **Даты — `_parse_date` (`DD.MM.YYYY` → `YYYY-MM-DD`).** Диапазонная выставка
  (`starts_at != ends_at`) → `date = ends_at`: slug стабилен между прогонами, карточка видна, пока
  выставка активна, а `cleanup_old_events` удалит её, когда `ends_at < сегодня`. Одиночное событие
  (или `ends_at` пуст/равен `starts_at`) → `date = starts_at`.
- Записи без `title_extended`/`starts_at` (новости, архив в ленте `/events/sub/home`) пропускаются
  с `permm.skipped_item`.
- venue_name/address берутся из seeds (в JSON их нет); цена 0 / «по билетам».

### `sources/permopera.py` — Театр оперы и балета (direct_api)

**`PermOperaClient`** — афиша месяца отдаётся эндпоинтом
`GET /playbills/playbill?month=YYYY-MM-01&front=1&json=1`, но это **HTML-в-JSON**: ответ —
`{"content": "<html-карточки>"}`. Карточки парсятся `selectolax`, без LLM и без браузера.

- ⚠️ Эндпоинт требует браузерных заголовков (`X-Requested-With: XMLHttpRequest` + `Referer`),
  иначе отдаёт **HTTP 400** (ловится `_run_direct_api_source` → warning/Telegram-алерт).
- `search` тянет **текущий и следующий месяц** (~60 дней афиши), дедуп по `source_url`.
- **`parse_cards(html, event_type, venue_name, address, today)`** — чистая функция (тест без сети).
  Стабильные якоря разметки (ЧИНИТЬ ТУТ при смене вёрстки): `article[data-element="event-card"]`,
  `link[data-element="event-date"] content` (ISO дата-время), `a[href*="/playbills/playbill/"]`
  (ссылка на спектакль + его название). Прошедшие спектакли (`date < today`) отсекаются.
- venue_name/address из seeds; цена 0 / «по билетам».
...
- **`discover_sources(city, supabase, *, settings, dry_run)`** — точка входа: гоняет типовые запросы
  ...
  `экскурсии {city}`, `фестиваль {city}`, `куда сходить {city}`), агрегирует домены, считает score,
  пишет в БД. Запросы `экскурсии`/`фестиваль`/`куда сходить` добавлены под слабые категории (⚠️ в
  таблице покрытия) + широкий fallback-запрос. Возвращает `(candidates, all_providers_failed)` —
  второй флаг `True`, если **все** keyed-провайдеры (Serper/Brave) ушли в `_disabled` (401/403/повторные
  отказы); DuckDuckGo без ключа в расчёт не берётся, чтобы его «вечная» поломка не маскировала битые ключи.
- **Скоринг кандидата:** `+3` если на странице найден JSON-LD типа `Event`; `+2` если в пути есть
  ...
  `−5` (`_AGGREGATOR_PENALTY`) если домен в `_AGGREGATOR_PENALTY_DOMAINS`. Штраф применяется
  **последним**, после всех начислений (иначе новые бонусы нивелировали бы его).
- **Два уровня отсева доменов** (общий хелпер `_match_domain_set(domain, set)` — суффиксное сравнение
  по иерархии: `kassir.ru` в сете ловит `perm.kassir.ru`, но `afisha.yandex.ru` в сете НЕ трогает
  `yandex.ru`; `_normalize_domain` снимает регистр и `www.`):
  - **Жёсткий фильтр `_IGNORE_DOMAINS`** (`_is_ignored`) — домен и все его поддомены выбрасываются
    из обработки в `collect_candidates`. Сюда входят соцсети/поисковики/2ГИС **и федеральные
    билетные платформы** (`ticketland.ru`, `kassir.ru`, `kassy.ru`, `afisha.ru`, `tripadvisor.ru`):
    они дублируют Timepad (`unique_events_ratio ≈ 0`), держать их кандидатами незачем. Суффиксное
    сравнение чинит «протекание» поддоменов (`m.vk.com`, `perm.kassir.ru`, `afisha.yandex.ru`),
    которое было при точном сравнении.
  - **Мягкий штраф `_AGGREGATOR_PENALTY_DOMAINS`** (`−5`) — домен остаётся кандидатом, но с
    пониженным score (виден в `discovery-health`, модератор может одобрить вручную). Сейчас в списке
    только `timepad.ru` (временно, пока `_known_domains()` не научится ловить `direct_api`-источники
    без `url` в seeds). Штраф `−5` ≥ суммы двух сильнейших бонусов (`+3`+`+2`) — гарантированно
    опускает агрегатор ниже организатора. Лог `candidate.aggregator_penalty` пишет
    `score_before`/`score_after`/`penalty`.
- **Фильтры:** пропускаются домены из `seeds.yaml`, домены со статусом `rejected`, `_IGNORE_DOMAINS`.
- **Авто-одобрение (`_should_auto_approve`)** — первоисточник с сильными сигналами одобряется
  **без ручной модерации**, чтобы очередь не стала узким местом. Критерии (все вместе): найден
  JSON-LD `Event` (`has_jsonld_event`), есть событийный путь в `sample_urls`, домен встретился в
  **≥3 запросах**. Исключены агрегаторы (`_AGGREGATOR_PENALTY_DOMAINS`) и **платформы-конструкторы**
  (`_BUILDER_DOMAINS`: `tilda.ws`, `wixsite.com`, `ucoz.ru`, …) — у конструктора домен второго уровня
  не идентифицирует организатора, generic не должен брать его как один источник.
- **`save_candidates`** сохраняет с **сохранением статуса** и обновляет `last_seen`. Статус:
  - **новый домен** (INSERT) → `approved`, если `_should_auto_approve`, иначе `new`;
  - **существующий `new`** (UPDATE) → апгрейд до `approved`, если домен *теперь* набрал критерии
    (накопительный эффект: был 1 запрос, стало 4) — закрывает «зависание» прогрессирующих доменов в
    вечном `new`. Лог `candidate.auto_approved` пишется **только в момент реального перехода**
    `new → approved`, не на каждом прогоне крона;
  - **существующий `approved`/`rejected`** — статус не трогается (решение модератора или прошлый
    авто-апрув приоритетны). У новых доменов фиксируется `first_provider`/`first_query`.
...
- **`discovery-health`** (CLI-команда) — за окно `--days` (дефолт 7) считает новых кандидатов по
  ...
  (раннее обнаружение поломки Discovery). Запускается шагом в `discover_sources.yml` — теперь
  **без** `continue-on-error`, поэтому 0 новых кандидатов за неделю валит джобу и шлёт Telegram-алерт.
...
`discovery-health --city {city}`. Секреты: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`,
`SERPER_API_KEY` (обязателен для рабочего поиска), `BRAVE_API_KEY` (опц., резерв);
`SEARCH_PROVIDERS` задаётся в `env:` workflow. Результат — кандидаты в `candidate_sources`.

**Алерты при сломанном поиске (две линии защиты):**
- **Быстрая (`discover-sources` → exit 1):** если все keyed-провайдеры (Serper/Brave)
  отключились из-за 401/403/повторных отказов, CLI возвращает 1 → джоба падает → срабатывает
  шаг **Notify on failure**. Сигнал «ключи API битые» приходит сразу, в тот же прогон.
  Пустой список кандидатов при живых ключах (узкая ниша, нет новых доменов) **не** считается
  ошибкой — exit 0, ложной тревоги нет.
- **Медленная (`discovery-health` → exit 1):** если за `--days` (дефолт 7) не появилось ни
  одного нового кандидата — медленная деградация поиска. Раньше шаг был помечен
  `continue-on-error: true` и сбой проглатывался; теперь он валит джобу и шлёт алерт.
...
- Рендер: `<BackLink>` вместо статичной ссылки · hero-изображение 21:9 (или градиентная emoji-заглушка) · бейдж типа · название · площадка · описание · блок адреса · «Когда» и «Организатор» · цена + кнопка «Перейти к источнику» · `<OtherDates>` (блок «Другие даты» серии, если у события есть повторы)
...
- `getSeriesSiblings(city, event, today)` — остальные сеансы той же серии (тот же `seriesKey`, что и карточка «ещё N дат» на главной) для блока «Другие даты»; события города на сборку кэшируются в модуле (`Map<string, Promise<EventItem[]>>` по ключу `city|today`), чтобы каждая из ~400 страниц событий не тянула город заново
- `rowToEvent()` — snake_case строка БД → camelCase `EventItem` TypeScript
- ре-экспортирует `getCityToday` из `lib/dateUtil.ts` (см. ниже), чтобы шесть существующих импортов из `events.ts` не трогать

**`lib/filters.ts`** — фильтрация серий событий (`lib/series.ts`) без запросов к серверу:
- По типу события: пустой набор `types` означает «все типы» — клик по чипу при умолчании выбирает только его, а не убирает единственный доступный тип
- По цене — пересечение диапазона события с диапазоном фильтра; `priceMax: null` означает «без верхней границы»
- `inDateSel(date, sel, today)` — попадание даты события в выбор ленты дат: `today` / `tomorrow` / `weekend` (ближайшие Сб+Вс, **включая сегодня**; в воскресенье — только сегодня, не следующая суббота) / `all` / конкретный `YYYY-MM-DD`
- `visibleSeries(series, filters, sel, today)` — для каждой серии выбирает первый сеанс, прошедший **и** фильтры, **и** окно дат (не первый по дате вообще)
...
- `DEFAULT_FILTERS` — `date: null` (умолчание — лента сама выбирает ближайший непустой день), пустой набор типов, цена 0–∞. SSR-mismatch невозможен: серверный prerender использует именно их, а URL читается только на клиенте

**`lib/dateUtil.ts`** — арифметика над строками `YYYY-MM-DD` через UTC-компоненты (не зависит от таймзоны сервера сборки или браузера): `addDaysUTC(ymd, n)`, `formatDayMonth(ymd)` («8 августа»), `weekdayUTC(ymd)`, `formatStripDay(ymd)` («Пн 21»), `formatWeekdayDayMonth(ymd)` («Сб, 19 сентября»). `getCityToday(city)` — **единственное место в кодовой базе**, где текущий момент превращается в календарную дату: считает «сегодня» в таймзоне города (`perm` → `Asia/Yekaterinburg`, `sochi` → `Europe/Moscow`), а не в UTC сервера сборки (билд идёт в 21:00 UTC).

**`lib/series.ts`** — группировка повторов одного события (разные даты одной выставки/спектакля) в серию на клиенте: `seriesKey(event)` — ключ `название (без возрастной метки) | площадка`, консервативно (лишняя карточка безопаснее слияния разных событий); `groupSeries(events)` — серии в порядке первого появления ключа; `otherDates(series, shown)` — сколько ещё **различных** дат у серии и до какой идёт; `groupByDate(items, dateOf)` — группировка подряд идущих элементов по дате (используется и лентой, и блоком «Другие даты»).

**`lib/dateStrip.ts`** — пункты ленты дат: `buildDateStrip(today)` → Сегодня/Завтра/Выходные/14 отдельных дней/Все даты; `firstNonEmptyDay(items, counts)` — первый отдельный день с событиями (умолчание, если в URL дата не задана).

**`lib/urlState.ts`** — состояние главной в URL `?date=&type=&pmin=&pmax=&q=`: `parseState(search, today)` откатывается к умолчанию по каждому полю независимо (прошедшая/несуществующая/мусорная дата, неизвестный тип, нечисловая цена не роняют страницу); `serializeState(filters, query)` не пишет значения по умолчанию и сортирует типы в порядке `ALL_TYPES`; `listStateKey(city)` — ключ `sessionStorage`, которым главная делится с страницей события для ссылки «← Все события».
...
**`CityView.tsx`** — главная города (client component, получает готовое `today` пропом со сборки):
- Hero: надзаголовок «Куда сходить сегодня», заголовок `{heroPrefix} {Город} ждёт» и счётчик серий в афише
- После монтирования пересчитывает «сегодня» (`getCityToday`, на случай устаревшей сборки) и читает `?date=&type=&pmin=&pmax=&q=` из URL (`lib/urlState.parseState`) — статический HTML один на все query-строки, поэтому раньше монтирования состояние читать нельзя
- `<FilterBar>` с типами по частоте, `<DateStrip>` со счётчиком серий на каждый пункт
- Секции по дням (`DaySection`) с заголовком `formatWeekdayDayMonth` и счётчиком-пилюлей; каждая карточка — серия (`lib/series.groupSeries` + `lib/filters.visibleSeries`), схлопывающая повторы одного события в одну карточку с «ещё N дат»
- Состояние пишется синхронно при клике по фильтру/ленте (`history.replaceState` + `sessionStorage[urlState.listStateKey(city)]`, в `try/catch`) и с задержкой 300 мс при наборе текста в поиске
- `EmptyState` предлагает ближайший непустой день и/или сброс фильтров + SEO-текстовый блок для индексации

**`FilterBar.tsx`** — горизонтальная панель фильтров над лентой дат:
- Поле поиска (ранжированный поиск по событиям и площадкам, `lib/search.ts`; Escape и ✕ очищают)
- Чипы типов: первые 4 видимы, остальные — в поповере «Ещё N»; пустой выбор = все типы, есть выбранные — кнопка «× Сбросить»
- Поповер «Ещё фильтры»: диапазон цены (верхняя граница может быть пустой — «любая»)

**`DateStrip.tsx`** — лента дат под `FilterBar`: Сегодня / Завтра / Выходные / 14 отдельных дней / Все даты, у каждого пункта число серий при текущих фильтрах; пустые дни приглушены и недоступны для клика (кроме уже выбранного), во время поиска ни один пункт не подсвечен и рядом показывается «Поиск по всем датам».

**`EventCard.tsx`** — карточка события/серии в сетке: изображение (через `CardImage`), бейдж типа, название, площадка, описание в 2 строки, до 2 тегов, дата/цена/адрес; для серии — строка «ещё N дат до …» (`moreDates` из `lib/series.otherDates`).
...
**`OtherDates.tsx`** (server component) — блок «Другие даты» на странице события: остальные сеансы серии (`lib/events.getSeriesSiblings`) сгруппированы по дате (`lib/series.groupByDate`), текущая дата помечена «эта дата» вместо ссылки. Если сиблингов нет — не рендерится.

**`BackLink.tsx`** (client component) — ссылка «← Все события»: после монтирования читает `sessionStorage[urlState.listStateKey(city)]` (в `try/catch` — приватный режим не должен ронять страницу) и ведёт на `/{city}` с теми же фильтрами, что были на главной перед переходом; без сохранённого состояния (прямой заход) — на `/{city}` без параметров.
...
**5. Discovery по дырявым категориям** ✅ *(PR #2)*  
В `QUERY_TEMPLATES` добавлены запросы под категории ⚠️: `экскурсии {city}`, `фестиваль {city}`,
`куда сходить {city}` (плюс ранее были `мастер-класс`/`детские мероприятия`).
Результат — кандидаты в `candidate_sources` → модерация (вкл. авто-апрув) → `seeds.yaml` или generic.
...
**7. Запустить generic-парсер** ✅ (Пермь) / 🔧 (Сочи — включён, ждёт approved-доменов)  
...
Для Сочи `generic` тоже включён в `seeds.yaml` (PR #2) — отработает вхолостую, пока
`discover-sources --city sochi` + авто-апрув не наполнят `candidate_sources` одобренными доменами.
...
**PR #2 (ветка `feat/discovery-pr2`) — сделано** 🔧

Все изменения — в `sources/candidate_sources.py` (детали выше в разделе модуля):

- **Суффиксная фильтрация доменов** — общий хелпер `_match_domain_set(domain, set)` сравнивает по
  иерархии (`kassir.ru` в сете ловит `perm.kassir.ru`, но `afisha.yandex.ru` не трогает `yandex.ru`);
  `_normalize_domain` снимает регистр и `www.`. Чинит «протекание» поддоменов (`m.vk.com`,
  `afisha.yandex.ru`), которое было при точном сравнении. `_IGNORE_DOMAINS` расширен билетными
  платформами (`ticketland.ru`, `kassir.ru`, `kassy.ru`, `afisha.ru`, `tripadvisor.ru`) — они
  переехали из мягкого штрафа в жёсткий фильтр (дублируют Timepad, `unique_events_ratio ≈ 0`).
  `_AGGREGATOR_PENALTY_DOMAINS` теперь — только `timepad.ru`.
- **Полуавтоматическая модерация (`_should_auto_approve`)** — первоисточник (`has_jsonld_event` +
  event-path + ≥3 запроса И **не** агрегатор И **не** платформа-конструктор `_BUILDER_DOMAINS`)
  одобряется автоматически. В `save_candidates` апрув применяется и к новым доменам (INSERT), и к
  существующим со статусом `new` (накопительный эффект: домен дозрел до критериев на N-м прогоне) —
  закрывает «зависание» прогрессирующих доменов в вечном `new`. Статусы модератора
  (`approved`/`rejected`) не перетираются; лог `candidate.auto_approved` — только в момент перехода.
- **Запросы под слабые категории** — `QUERY_TEMPLATES` дополнен `экскурсии {city}`, `фестиваль {city}`,
  `куда сходить {city}` (закрывает п. 5 roadmap и категории ⚠️).
- **Generic включён для Сочи** — раскомментирован в `seeds.yaml` (priority 20). Безопасно при пустом
  списке approved-доменов (0 доменов → 0 LLM-вызовов); самоактивируется после `discover-sources --city sochi`.
- **Тесты** — 19 новых в `tests/test_candidate_sources.py` (суффиксная фильтрация: поддомены, регистр,
  `www.`, направление иерархии, отсутствие ложных срабатываний; auto-approve по всем критериям; save с
  разными статусами через мок Supabase). 5 тестов в `tests/test_discovery.py` переведены с удалённых
  пенальти-доменов на `timepad.ru`. Все 165 тестов зелёные.

> **Боевой прогон — сделано (2026-09-18).** `discover-sources --city perm` (с `SERPER_API_KEY`) отдал
> 38 доменов, avg_score 0.8, score≥7 = 0; в топе нет `ticketland.ru`/`kassir.ru`/`afisha.ru`/
> `tripadvisor.ru` — суффиксная фильтрация агрегаторов подтверждена на живых данных.
> `discovery-health --city perm` вернул exit 1 (0 новых кандидатов за 7 дн.) — **известное
> ограничение, не баг**: здоровье считает по `found_at` (момент первого обнаружения), а фиксированный
> набор из 8 query-шаблонов уже 3 месяца возвращает один и тот же топ выдачи — все 63 домена
> Перми в `candidate_sources` найдены 21.06.2026, новых сверх этого списка не появилось. Красный
> статус сигналит не о протухшем ключе/сломанном пайплайне, а о том, что текущим запросам пора
> добавить разнообразия (домены Перми для этих формулировок исчерпаны).

**Прочее (вне Discovery):**
- **Спец-источники для SPA-площадок** ✅ — `permopera.ru` / `permm.ru` (афиша за JS) подключены
  через режим **`direct_api`**: у обоих нашлись открытые JSON-эндпоинты, поэтому вместо браузерного
  рендера (`playwright_listing`) парсим JSON напрямую — без LLM, без Playwright, без проблем SSL/404.
  Клиенты `sources/permm.py`, `sources/permopera.py`. Следующие кандидаты: `permtuz.ru`,
  `teatr-umosta.ru` (`playwright_listing`, либо `direct_api` если найдётся JSON-API).

### Backlog: унификация direct_api (потенциальные улучшения)

После добавления permm/permopera накопились идеи на будущее (не реализованы — избыточны для текущего
числа клиентов, заводятся отдельными задачами при необходимости):

| Идея | Зачем | Почему не сейчас |
|------|-------|------------------|
| `FetchedEvent`/`DirectApiItem` вместо `tuple[ParsedEvent, str]` | место под `source_id`, `etag`, `checksum`, `updated_at` | рефактор всех direct_api-клиентов (quizplease/timepad/twogis/permm/permopera) + диспетчера |
| Унифицированный `DirectApiClient` (fetch → map → optional enrich) | убрать дублирование между клиентами | имеет смысл после ≥3 схожих клиентов |
| Enrichment с детальных страниц (точная цена, полное описание) | богаче карточки permm/permopera | +N запросов; нужен кэш 24h + checksum; MVP обходится листингом |
| `source_id` (external_id из JSON) в БД | дебаг (`permm-7812`), идемпотентность | требует миграцию схемы `events` |
| `checksum sha256(raw_json)` | пропускать парсинг detail, если не менялось | нужен только вместе с enrichment |
| Schema-drift guard (`API_VERSION` + лог unexpected/missing полей) | эти JSON меняют тихо | отдельная кросс-источниковая задача |
| Отдельные `event_date` + `active_until` в модели | честная семантика диапазона выставок | миграция модели; пока закрывает `date = ends_at` |

---

#### Аналитика и инструменты

**Мониторинг тихих сбоев (токены/квоты)** ✅ *(2026-06-25)*  
Раньше два класса сбоев были невидимы: (а) протухший `TIMEPAD_TOKEN` (HTTP 403) ловился общим
`except` в `_run_direct_api_source`, прогон оставался зелёным — событий просто не было;
(б) исчерпание/битьё ключей Serper/Brave в Discovery проглатывалось из-за `continue-on-error`.
Сделано:
- **`PipelineResult.warnings`** — источник копит критичные-но-не-фатальные проблемы в своём
  локальном `sub` (изоляция: сбой одного не «заражает» соседей), `run_city` агрегирует.
- **Ветка HTTP 400/401/403** в `_run_direct_api_source` пишет понятный текст в `warnings`
  и в `source_health.last_error` (видно в БД, какой источник «болеет»). Тест изоляции —
  `tests/test_pipeline_warnings.py`.
- **`parse.yml` → шаг «Send warning alert»** (`if: success()`): читает `parse_warnings_<city>.json`
  и шлёт Telegram через `scripts/notify_warnings.py` (только stdlib, экранирование Markdown,
  `timeout=10`). Алерт приходит, даже когда джоба формально успешна.
- **Discovery exit 1** — `discover_sources()` возвращает `(candidates, all_providers_failed)`;
  `discover-sources` падает при провале **всех keyed-провайдеров** (быстрый сигнал «ключи битые»),
  но **не** при пустой выдаче на живых ключах (нет ложных тревог в узких нишах). С `discovery-health`
...
через `dedup-backfill`. Скорер откалиброван на живых данных (691 карточка Перми): предпросмотр
бэкфилла при пороге 0.95 сжался с 69 схлопываний в 40 кластеров (максимум — 9) до **46 в 37**
(максимум — 4), закрыв три семейства ложных слияний — перечни, зонтичные программы и самодубли
источника (см. раздел «`fuzzy.py` + `merge.fuzzy_merge`» и спеку). Осталось нереализованным:
...
- разбор накопленного: `dedup-backfill --city perm --threshold 0.95 --apply` ещё не запускался
  (удаление 46 боевых карточек — осознанный ручной шаг, риск 404 на проиндексированных URL)
...
районами Перми не пережили правку). Покрытие поля замерено (2026-09-21): `district` заполнен
у 3 из 359 будущих событий Перми. Фильтр по району упирается не в вёрстку, а в парсер — без
источника, который отдаёт район, строить список районов не из чего.

`category` — тоже не опора для фильтров: заполнена у 35 из 359 событий свободным текстом
(«6+», «english club», «Драма», «Мастер-класс», «art»), единого словаря нет, LLM пишет что хочет.
Не путать с `EventType` (`_map_category` в `parser/src/parser/sources/timepad.py`) — это
маппинг категорий Timepad в закрытый набор типов события, к полю `category` не относится.

**19. Мобильная вёрстка ленты** 📋 *(отложено из UX-разбора 2026-09-19)*  
Замеры на 375px (`/perm`): фильтры занимают весь первый экран — первая карточка начинается на
~525px; карточки идут в 2 колонки по ~161px при высоте ~509px, названия обрезаются
(«Лекция «Юдифь и…»); страница целиком ~114 000px. Варианты к проработке (с макетами):
компактная карточка в 1 колонку (миниатюра слева, время → название → площадка → цена, без описания)
и липкая компактная панель фильтров («Когда» + кнопка «Фильтры · N», остальное — в bottom sheet).
Делать после ленты дат и схлопывания серий — они меняют, что именно показывается на экране.
...
**18. Разделить «бесплатно» и «цена неизвестна»** 📋  
...
SEARCH_PROVIDERS=serper,brave            # порядок = приоритет; duckduckgo отдаёт 202 антибот с 2026-06
