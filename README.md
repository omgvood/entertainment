...
**Наполнение** — `parser-cli refresh-venues --city <c>` (workflow `refresh_venues.yml`, раз в 2 недели).
2ГИС Catalog API — primary; `sources/playwright_2gis.py` (браузерный сбор) — fallback при ошибке/пустом
ответе API либо явным `--source playwright`. Запросы и типы берутся из `twogis`-источников в `seeds.yaml`.
...
Фронтенд сейчас читает постоянные места из `events` (`date='always'`, источник 2ГИС). Нужно:
- Добавить в `lib/events.ts` запрос к таблице `venues`
- Вывести страницы площадок (`/perm/venues/[slug]/`) с SEO
- Убрать 2ГИС из ежедневного `parse.yml` (больше не нужен для событий)
...
| `refresh-venues --city <c\|all> [--source auto\|twogis\|playwright]` | Сбор заведений в таблицу `venues` (2ГИС) | да (`venues`) |
