...
| Проблема про способ работы, а не про код: «с чего начать», «что дальше», аудит закрытой волны, разбор чужой сессии | `process-review` — **вне git**, пользовательский скилл в `~/.claude/skills/`: обслуживает и процессы за пределами этого репозитория | — (вердикт текстом, скилл только читает) |
