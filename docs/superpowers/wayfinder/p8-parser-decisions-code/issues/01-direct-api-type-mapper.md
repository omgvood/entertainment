...
Тесты: `test_permopera.py` — метка → тип (4 кейса на снимке живой афиши `fixtures/permopera_playbill_genres.json`), фолбэк на незнакомую метку, старое `type == "concert"` переписано на `theater` (карточка с меткой «Опера»); `test_permm.py` — кейсы правила фида событий на заголовках из БД (включая заголовок капсом) и `search()` через `httpx.MockTransport` на фикстурах обоих фидов (без флага `from_events_feed` падает — проверено мутацией). `pytest` — 298 passed.

Ревью PR #35 (`code-review`, high): до мерджа исправлены регистр в проверке «Выставка» (permm публикует часть заголовков капсом), устаревший докстринг `SourceConfig.event_type` и непокрытая провязка флага в `search()`. Оставлены сознательно: при смене вёрстки permopera тип молча уходит в фолбэк `theater` (лога нет); у `trip`/`other` нет авто-тегов в `TYPE_DEFAULT_TAGS` — экскурсии/«другое» permopera и не-выставки permm после прогона останутся без авто-тегов, это территория тикета 02; фикстура `permopera_playbill_genres.json` ~148 КБ (полные карточки с составами); в `vk.py` есть свой классификатор по ключевым словам — правило permm с ним не объединено.
