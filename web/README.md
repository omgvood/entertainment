...
- `date >= сегодня` — прошедшие события не показываются. «Сегодня» считается по местной
  таймзоне города (`Asia/Yekaterinburg` / `Europe/Moscow`), а не по UTC сервера сборки
- Постоянные места (`date='always'`) живут в таблице `venues`, а не `events` — на главной их
  показывает `VenuesSection`, полный список — `/{city}/venues/`
