...
class SourceConfig:
    ...
    """Тип событий источника по умолчанию. twogis ставит его всем событиям; permopera
    и permm определяют тип по событию и берут этот только как фолбэк; для JSON-LD — дефолт,
    когда Schema.org @type не уточняет тип."""
