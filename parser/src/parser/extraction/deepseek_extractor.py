"""DeepSeek V4 Flash экстрактор события через OpenRouter API.

OpenRouter предоставляет унифицированный API для DeepSeek и других моделей.
Интеграция аналогична Groq (OpenAI-совместимый API).
"""
...
class DeepSeekExtractor(LLMExtractor):
    ...
    async def extract(self, html: str, source_url: str) -> ParsedEvent:
        ...
        if not text:
            raise ExtractorError(f"DeepSeek вернул пустой ответ для {source_url}")
    ...
    async def extract_many(self, html: str, source_url: str) -> list[ParsedEvent]:
        ...
        if items is None:
            raise ExtractorError(
                f"DeepSeek не вернул events-массив для {source_url}: keys={list(data.keys()) if isinstance(data, dict) else type(data).__name__}"
            )
...
def _clean_html(html: str, max_chars: int = 20_000) -> str:
    """Жмём HTML до Markdown для снижения токен-использования.
