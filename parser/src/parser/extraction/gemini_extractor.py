...
- organizer — название организатора/компании-устроителя (например 'QuizPlease', 'Пермская филармония'), если явно указано на странице; иначе null.
...
- tags — выбери подходящие из ЗАКРЫТОГО набора (можно несколько, можно ни одного): "для компании", "для пары", "для детей", "интеллектуальное", "активное", "творческое", "вечером", "днём", "в помещении", "на улице", "бесплатно". НЕ придумывай свои теги — всё, что вне набора, будет отброшено.
...
- organizer — название организатора/компании-устроителя (например 'QuizPlease', 'Пермская филармония'), если явно указано; иначе null.
- event_url — прямая ссылка на страницу именно этого события. Два варианта: (1) в HTML у карточки есть <a href="..."> — возьми его; (2) перед текстом события есть маркер «=== POST <url> ===» — используй этот url. Иначе null.
  ЗАПРЕЩЕНО придумывать или угадывать event_url. Никогда не инкрементируй ID из URL других постов (плохо: посты .../1, .../2 → ты генерируешь .../3; хорошо: нет явной ссылки → null). Если маркер «=== POST <url> ===» отсутствует и в HTML нет явного <a href> на конкретное событие — event_url обязан быть null.
